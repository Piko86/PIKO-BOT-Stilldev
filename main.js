...
const { interactiveMenuCommand, handleMenuNavigation, menuStatusCommand } = require('./commands/interactivemenu');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // Check for interactive menu navigation first (for reply-based navigation)
        if (!userMessage.startsWith('.')) {
            // Try to handle menu navigation first
            const menuHandled = await handleMenuNavigation(sock, chatId, message, userMessage, senderId);
            if (menuHandled) {
        ...
        switch (true) {
            ...
            case userMessage === '.menu':
                ...
                commandExecuted = true;
                break;
            case userMessage === '.menustatus':
                await menuStatusCommand(sock, chatId, message);
                commandExecuted = true;
                break;
